...
object RandomGame2048Initializer: Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        ...
        val container: Pair<Cell, Int>
        while (true) {
            val i = generateRandomStartValue()
            val j = generateRandomStartValue()

            if (board[Cell(i, j)] != null) {
                continue
            }

            container = Cell(i, j) to generateRandomStartValue()
            break
        }
